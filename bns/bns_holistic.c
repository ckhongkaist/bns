...
void getPredecessors(Solver& S, int num_var, int curr_depth) {
    vec<Lit> lits;
    int i = num_var;
    ...
    */
    ...
    int number_of_attr = global_var.Attractors.size();
    ...
    while (1){
      ...
      char tState[num_var];
      while (i<num_var) {
        //sprintf(tState+i, "%c", (S.model[i] == l_True)?'1':'0');
        tState[i] = (S.model[i]==l_True)?'1':'0';
      ...
      i = number_of_attr;
      while (i--) {
        int nStates = global_var.Attractors[i].length;
        bool found = false; 
        while (nStates--) {
            if(!strcmp(tState, global_var.Attractors[i].states[nStates].c_str())) {
                global_var.Attractors[i].nBasins++;
                found = true;
                break;
            }
        }
...
/* ####### Basin Analysis ####### */
  ...
  //construct_depth(number_of_var);
  //int tSize = getPredecessors(S_, attr_num, number_of_var, number_of_var);
  attr_num = 0;
  ...
  printf("\n");
  getPredecessors(S_, number_of_var, curr_depth);
  ...
  for (i=0; i<global_var.Attractors.size(); i++) {
    printf("Basin size for attractor %d: %d\n", i, global_var.Attractors[i].nBasins);
    total_basin_size = total_basin_size + global_var.Attractors[i].nBasins;
  }

  printf("Total basin size: %d\n", total_basin_size);
  return 0;
